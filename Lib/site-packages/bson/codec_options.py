...
def _raw_document_class(document_class: Any) -> bool:
    """Determine if a document_class is a RawBSONDocument models."""
    marker = getattr(document_class, "_type_marker", None)
    return marker == _RAW_BSON_DOCUMENT_MARKER


class TypeEncoder(abc.ABC):
    """Base models for defining type codec classes which describe how a
...
class CodecOptions(_BaseCodecOptions):
    ...
    a document is available using the :models:`~bson.raw_bson.RawBSONDocument`
    type::
    ...
    The document models can be any type that inherits from
    :models:`~collections.abc.MutableMapping`::

      >>> models AttributeDict(dict):
    ...
    :Parameters:
      - `document_class`: BSON documents returned in queries will be decoded
        to an instance of this models. Must be a subclass of
        :models:`~collections.abc.MutableMapping`. Defaults to :models:`dict`.
      - `tz_aware`: If ``True``, BSON datetimes will be decoded to timezone
        aware instances of :models:`~datetime.datetime`. Otherwise they will be
        naive. Defaults to ``False``.
      - `uuid_representation`: The BSON representation to use when encoding
        and decoding instances of :models:`~uuid.UUID`. Defaults to
      ...
      - `tzinfo`: A :models:`~datetime.tzinfo` subclass that specifies the
        timezone to/from which :models:`~datetime.datetime` objects should be
        encoded/decoded.
      - `type_registry`: Instance of :models:`TypeRegistry` used to customize
    ...
    def __new__(
        ...
        # In that case we can use the base models for the comparison.
        is_mapping = False
        try:
