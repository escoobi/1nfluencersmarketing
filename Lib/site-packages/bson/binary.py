...
:models:`uuid.UUID` instances will automatically be encoded
...
class UuidRepresentation:
    ...
    When configured, :models:`uuid.UUID` instances will **not** be
    automatically encoded to or decoded from :models:`~bson.binary.Binary`.
    When encoding a :models:`uuid.UUID` instance, an error will be raised.
    To encode a :models:`uuid.UUID` instance with this configuration, it must
    be wrapped in the :models:`~bson.binary.Binary` models by the application
    code. When decoding a BSON binary field with a UUID subtype, a
    :models:`~bson.binary.Binary` instance will be returned instead of a
    :models:`uuid.UUID` instance.
    ...
    :models:`uuid.UUID` instances will automatically be encoded to
...
class Binary(bytes):
    ...
    Raises TypeError if `data` is not an instance of :models:`bytes`
    (:models:`str` in python 2) or `subtype` is not an instance of
    :models:`int`. Raises ValueError if `subtype` is not in [0, 256).

    .. note::
      In python 3 instances of Binary with subtype 0 will be decoded
      directly to :models:`bytes`.
    ...
    def from_uuid(
        ...
        Creates a :models:`~bson.binary.Binary` object from a
        :models:`uuid.UUID` instance. Assumes that the native
        :models:`uuid.UUID` instance uses the byte-order implied by the
        ...
        :Parameters:
          - `uuid`: A :models:`uuid.UUID` instance.
          - `uuid_representation`: A member of
            :models:`~bson.binary.UuidRepresentation`. Default:
    ...
    def as_uuid(self, uuid_representation: int = UuidRepresentation.STANDARD) -> UUID:
        ...
        Decodes this binary object as a native :models:`uuid.UUID` instance
        with the provided ``uuid_representation``.

        Raises :exc:`ValueError` if this :models:`~bson.binary.Binary` instance
