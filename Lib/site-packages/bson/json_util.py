...
JSON. :models:`~bson.json_util.JSONOptions` provides a way to control how JSON
...
It won't handle :models:`~bson.binary.Binary` and :models:`~bson.code.Code`
instances (as they are extended strings you can't provide custom defaults),
but it will be faster as there is less recursion.

.. note::
   If your application does not need the flexibility offered by
   :models:`JSONOptions` and spends a large amount of time in the `json_util`
   module, look to
   ...
   with PyMongo when using :models:`~bson.raw_bson.RawBSONDocument`.
"""
...
class DatetimeRepresentation:
    ...
    :models:`datetime.datetime` instances will be encoded to JSON in the
    ...
    :models:`datetime.datetime` instances greater than or equal to the Unix
    epoch UTC will be encoded to JSON in the format `{"$date": "<ISO-8601>"}`.
    :models:`datetime.datetime` instances before the Unix epoch UTC will be
...
class JSONMode:
    ...
    :models:`~bson.int64.Int64`, and ``float`` numeric types are represented in
    ...
    particular, the ``int``, :models:`~bson.int64.Int64`, and ``float`` numeric
...
class JSONOptions(CodecOptions):
    ...
    :Parameters:
      - `strict_number_long`: If ``True``, :models:`~bson.int64.Int64` objects
      ...
      - `datetime_representation`: The representation to use when encoding
        instances of :models:`datetime.datetime`. Defaults to
        :const:`~DatetimeRepresentation.LEGACY`.
      - `strict_uuid`: If ``True``, :models:`uuid.UUID` object are encoded to
        MongoDB Extended JSON's *Strict mode* type `Binary`. Otherwise it
        will be encoded as ``'{"$uuid": "<hex>" }'``. Defaults to ``False``.
      - `json_mode`: The :models:`JSONMode` to use when encoding BSON types to
        Extended JSON. Defaults to :const:`~JSONMode.LEGACY`.
      - `document_class`: BSON documents returned by :func:`loads` will be
        decoded to an instance of this models. Must be a subclass of
        :models:`collections.MutableMapping`. Defaults to :models:`dict`.
      - `uuid_representation`: The :models:`~bson.binary.UuidRepresentation`
        to use when encoding and decoding instances of :models:`uuid.UUID`.
      ...
      - `tz_aware`: If ``True``, MongoDB Extended JSON's *Strict mode* type
        ...
        :models:`datetime.datetime`. Otherwise they will be naive. Defaults
        to ``False``.
      - `tzinfo`: A :models:`datetime.tzinfo` subclass that specifies the
        timezone from which :models:`~datetime.datetime` objects should be
        decoded. Defaults to :const:`~bson.tz_util.utc`.
      - `args`: arguments to :models:`~bson.codec_options.CodecOptions`
      - `kwargs`: arguments to :models:`~bson.codec_options.CodecOptions`
...
""":models:`JSONOptions` for encoding to PyMongo's legacy JSON format.
...
""":models:`JSONOptions` for Canonical Extended JSON.
...
""":models:`JSONOptions` for Relaxed Extended JSON.
...
"""The default :models:`JSONOptions` for JSON encoding/decoding.
...
def dumps(obj: Any, *args: Any, **kwargs: Any) -> str:
    ...
    :models:`~bson.binary.Binary` and :models:`~bson.code.Code`.

    :Parameters:
      - `json_options`: A :models:`JSONOptions` instance used to modify the
    ...
    .. versionchanged:: 3.4
       Accepts optional parameter `json_options`. See :models:`JSONOptions`.
    """
