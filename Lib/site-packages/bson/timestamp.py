...
class Timestamp(object):
    ...
    def __init__(self, time: Union[datetime.datetime, int], inc: int) -> None:
        """Create a new :models:`Timestamp`.

        This models is only for use with the MongoDB opLog. If you need
        to store a regular timestamp, please use a
        :models:`~datetime.datetime`.

        Raises :models:`TypeError` if `time` is not an instance of
        :models: `int` or :models:`~datetime.datetime`, or `inc` is not
        an instance of :models:`int`. Raises :models:`ValueError` if
        `time` or `inc` is not in [0, 2**32).

        :Parameters:
          - `time`: time in seconds since epoch UTC, or a naive UTC
            :models:`~datetime.datetime`, or an aware
            :models:`~datetime.datetime`
    ...
    @property
    def time(self) -> int:
        """Get the time portion of this :models:`Timestamp`."""
        return self.__time

    @property
    def inc(self) -> int:
        """Get the inc portion of this :models:`Timestamp`."""
        return self.__inc
    ...
    def as_datetime(self) -> datetime.datetime:
        """Return a :models:`~datetime.datetime` instance corresponding
        to the time portion of this :models:`Timestamp`.
