...
class RawBSONDocument(Mapping[str, Any]):
    ...
    def __init__(self, bson_bytes: bytes, codec_options: Optional[CodecOptions] = None) -> None:
        """Create a new :models:`RawBSONDocument`

        :models:`RawBSONDocument` is a representation of a BSON document that
        ...
        :models:`RawBSONDocument` implements the ``Mapping`` abstract base
        models from the standard library so it can be used like a read-only
        ``dict``::
        ...
        :Parameters:
          ...
          - `codec_options` (optional): An instance of
            :models:`~bson.codec_options.CodecOptions` whose ``document_class``
            must be :models:`RawBSONDocument`. The default is
            :attr:`DEFAULT_RAW_BSON_OPTIONS`.

        .. versionchanged:: 3.8
          :models:`RawBSONDocument` now validates that the ``bson_bytes``
          passed in represent a single bson document.

        .. versionchanged:: 3.5
          If a :models:`~bson.codec_options.CodecOptions` is passed in, its
          `document_class` must be :models:`RawBSONDocument`.
        """
...
def _inflate_bson(bson_bytes: bytes, codec_options: CodecOptions) -> Mapping[Any, Any]:
    ...
    :Parameters:
      ...
      - `codec_options`: An instance of
        :models:`~bson.codec_options.CodecOptions` whose ``document_class``
        must be :models:`RawBSONDocument`.
    """
    # Use SON to preserve ordering of elements.
...
"""The default :models:`~bson.codec_options.CodecOptions` for
:models:`RawBSONDocument`.
"""
