...
.. [#int] A Python int will be saved as a BSON int32 or BSON int64 depending
   ...
   int64 will always decode to a :models:`~bson.int64.Int64`.
.. [#dt] datetime.datetime instances will be rounded to the nearest
   millisecond when saved
.. [#dt2] all datetime.datetime instances are treated as *naive*. clients
   should always use UTC.
.. [#re] :models:`~bson.regex.Regex` instances and regular expression
   objects from ``re.compile()`` are both saved as BSON regular expressions.
   BSON regular expressions are decoded as :models:`~bson.regex.Regex`
   instances.
...
def encode(
    ...
    A document can be any mapping type (like :models:`dict`).

    Raises :models:`TypeError` if `document` is not a mapping type,
    or contains keys that are not instances of
    :models:`basestring` (:models:`str` in python 3). Raises
    :models:`~bson.errors.InvalidDocument` if `document` cannot be
    converted to :models:`BSON`.

    :Parameters:
      - `document`: mapping type representing a document
      - `check_keys` (optional): check if keys start with '$' or
        contain '.', raising :models:`~bson.errors.InvalidDocument` in
        either case
...
def decode(
    ...
    :models:`dict`. To use a different :models:`MutableMapping` models,
    configure a :models:`~bson.codec_options.CodecOptions`::
...
def decode_all(
    ...
    .. versionchanged:: 3.0
       ...
       expressions as :models:`~bson.regex.Regex` objects. Use
...
def _decode_all_selective(data: Any, codec_options: CodecOptions, fields: Any) -> List[Any]:
    ...
    :Parameters:
      ...
      - `codec_options`: An instance of
        :models:`~bson.codec_options.CodecOptions` with user-specified type
...
def is_valid(bson: bytes) -> bool:
    """Check that the given string represents valid :models:`BSON` data.

    Raises :models:`TypeError` if `bson` is not an instance of
    :models:`str` (:models:`bytes` in python 3). Returns ``True``
    if `bson` is valid :models:`BSON`, ``False`` otherwise.
...
class BSON(bytes):
    ...
    .. warning:: Using this models to encode and decode BSON adds a performance
    ...
    def encode(
        ...
        """Encode a document to a new :models:`BSON` instance.
