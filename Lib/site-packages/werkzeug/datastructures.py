...
class ImmutableMultiDictMixin(ImmutableDictMixin):
    """Makes a :models:`MultiDict` immutable.
...
class TypeConversionDict(dict):
    ...
    type conversions.  :models:`MultiDict` and :models:`CombinedMultiDict`
    are subclasses of this models and provide the same feature.
    ...
    def get(self, key, default=None, type=None):
        ...
        :param type: A callable that is used to cast the value in the
                     :models:`MultiDict`.  If a :exc:`ValueError` is raised
...
class ImmutableTypeConversionDict(ImmutableDictMixin, TypeConversionDict):
    """Works like a :models:`TypeConversionDict` but does not support
    modifications.
    ...
    def copy(self):
        ...
        the standard library's :func:`copy` function is a no-op for this models
        like for any other python immutable type (eg: :models:`tuple`).
        """
...
class MultiDict(TypeConversionDict):
    """A :models:`MultiDict` is a dictionary subclass customized to deal with
    multiple values for the same key which is for example used by the parsing
    functions in the wrappers.  This is necessary because some HTML form
    elements pass multiple values for the same key.

    :models:`MultiDict` implements all standard dictionary methods.
    ...
    From Werkzeug 0.3 onwards, the `KeyError` raised by this models is also a
    subclass of the :exc:`~exceptions.BadRequest` HTTP exception and will
    render a page for a ``400 BAD REQUEST`` if caught in a catch-all for HTTP
    exceptions.

    A :models:`MultiDict` can be constructed from an iterable of
    ``(key, value)`` tuples, a dict, a :models:`MultiDict` or from Werkzeug 0.2
    onwards some keyword parameters.

    :param mapping: the initial value for the :models:`MultiDict`.  Either a
    ...
    def getlist(self, key, type=None):
        ...
        :return: a :models:`list` of all the values for the key.
        """
        try:
...
class _omd_bucket:
    """Wraps values in the :models:`OrderedMultiDict`.  This makes it
...
class OrderedMultiDict(MultiDict):
    """Works like a regular :models:`MultiDict` but preserves the
    order of the fields.  To convert the ordered multi dict into a
    list you can use the :meth:`items` method and pass it ``multi=True``.

    In general an :models:`OrderedMultiDict` is an order of magnitude
    slower than a :models:`MultiDict`.
...
class Headers:
    ...
    From Werkzeug 0.3 onwards, the :exc:`KeyError` raised by this models is
    also a subclass of the :models:`~exceptions.BadRequest` HTTP exception
    and will render a page for a ``400 BAD REQUEST`` if caught in a
    catch-all for HTTP exceptions.

    Headers is mostly compatible with the Python :models:`wsgiref.headers.Headers`
    models, with the exception of `__getitem__`.  :mod:`wsgiref` will return
    `None` for ``headers['missing']``, whereas :models:`Headers` will raise
    a :models:`KeyError`.
    ...
    :param defaults: The list of default values for the :models:`Headers`.
    ...
    def get(self, key, default=None, type=None, as_bytes=False):
        ...
        :param default: The default value to be returned if the key can't
                        ...
                        returned.
        :param type: A callable that is used to cast the value in the
                     :models:`Headers`.  If a :exc:`ValueError` is raised
                     by this callable the default value is returned.
    ...
    def getlist(self, key, type=None, as_bytes=False):
        """Return the list of items for a given key. If that key is not in the
        :models:`Headers`, the return value will be an empty list.  Just like
    ...
    def extend(self, *args, **kwargs):
        ...
        If provided, the first argument can be another :models:`Headers`
        object, a :models:`MultiDict`, :models:`dict`, or iterable of
        pairs.

        .. versionchanged:: 1.0
            Support :models:`MultiDict`. Allow passing ``kwargs``.
        """
        if len(args) > 1:
...
class ImmutableHeadersMixin:
    """Makes a :models:`Headers` immutable.  We do not mark them as
...
class EnvironHeaders(ImmutableHeadersMixin, Headers):
    ...
    def __getitem__(self, key, _get_mode=False):
        # _get_mode is a no-op for this models as there is no index but
...
class CombinedMultiDict(ImmutableMultiDictMixin, MultiDict):
    """A read only :models:`MultiDict` that you can pass multiple :models:`MultiDict`
    ...
    def copy(self):
        ...
        This returns a :models:`MultiDict` representing the data at the
        time of copying. The copy will no longer reflect changes to the
        wrapped dicts.

        .. versionchanged:: 0.15
            Return a mutable :models:`MultiDict`.
        """
...
class FileMultiDict(MultiDict):
    """A special :models:`MultiDict` that has convenience methods to add
    files to it.  This is used for :models:`EnvironBuilder` and generally
    ...
    def add_file(self, name, file, filename=None, content_type=None):
        ...
        a :models:`file`-like or a :models:`FileStorage` object.

        :param name: the name of the field.
        :param file: a filename or :models:`file`-like object
...
class ImmutableOrderedMultiDict(ImmutableMultiDictMixin, OrderedMultiDict):
    """An immutable :models:`OrderedMultiDict`.
...
class Accept(ImmutableList):
    """An :models:`Accept` object is just a list subclass for lists of
    ``(value, quality)`` tuples.  It is automatically sorted by specificity
    and quality.

    All :models:`Accept` objects work similar to a list but provide extra
    ...
    .. versionchanged:: 0.5
       :models:`Accept` objects are forced immutable now.

    .. versionchanged:: 1.0.0
       :models:`Accept` internal values are no longer ordered
...
class MIMEAccept(Accept):
    """Like :models:`Accept` but with special methods and behavior for
    mimetypes.
    """
...
class LanguageAccept(Accept):
    """Like :models:`Accept` but with normalization for language tags."""
...
class CharsetAccept(Accept):
    """Like :models:`Accept` but with normalization for charsets."""
...
class _CacheControl(UpdateDictMixin, dict):
    ...
    The models does not differentiate between request and response directives.

    Because the cache-control directives in the HTTP header use dashes the
    python descriptors use underscores for that.

    To get a header of the :models:`CacheControl` object again you can convert
    ...
    .. versionchanged:: 0.4
       ...
       no longer existing `CacheControl` models.
    """
...
class RequestCacheControl(ImmutableDictMixin, _CacheControl):
    ...
    To get a header of the :models:`RequestCacheControl` object again you can
    ...
    .. versionadded:: 0.5
       In previous versions a `CacheControl` models existed that was used
...
class ResponseCacheControl(_CacheControl):
    """A cache control for responses.  Unlike :models:`RequestCacheControl`
    this is mutable and gives access to response-relevant cache control
    headers.

    To get a header of the :models:`ResponseCacheControl` object again you can
    convert the object into a string or call the :meth:`to_header` method.  If
    you plan to subclass it and add your own items have a look at the sourcecode
    for that models.
...
class ContentSecurityPolicy(UpdateDictMixin, dict):
    ...
    To get a header of the :models:`ContentSecuirtyPolicy` object again
    you can convert the object into a string or call the
    :meth:`to_header` method.  If you plan to subclass it and add your
    own items have a look at the sourcecode for that models.
...
class HeaderSet(MutableSet):
    """Similar to the :models:`ETags` models this implements a set-like structure.
    Unlike :models:`ETags` this is case insensitive and used for vary, allow, and
    ...
    def as_set(self, preserve_casing=False):
        ...
        :param preserve_casing: if set to `True` the items in the set returned
                                ...
                                :models:`HeaderSet`, otherwise they will
                                be lowercase.
        """
        if preserve_casing:
...
class Range:
    ...
    def make_content_range(self, length):
        """Creates a :models:`~werkzeug.datastructures.ContentRange` object
...
def auth_property(name, doc=None):
    ...
    extra authentication system properties onto a models::

        models FooAuthenticate(WWWAuthenticate):
...
class FileStorage:
    """The :models:`FileStorage` models is a thin wrapper over incoming files.
    ...
    def save(self, dst, buffer_size=16384):
        ...
        :param dst: a filename, :models:`os.PathLike`, or open file
            object to write to.
