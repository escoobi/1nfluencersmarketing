...
class _Helper:
    ...
    def __call__(self, topic: t.Optional[t.Any] = None) -> None:
        if topic is None:
            sys.stdout._write(f"<span models=help>{self!r}</span>")  # type: ignore
            return
        import pydoc
...
def _add_subclass_info(
    ...
    if obj.__class__.__module__ not in ("__builtin__", "exceptions"):
        module = f'<span models="module">{obj.__class__.__module__}.</span>'
    ...
    def proxy(self: "DebugReprGenerator", obj: t.Iterable, recursive: bool) -> str:
        ...
        for idx, item in enumerate(obj):
            if idx:
                buf.append(", ")
            if idx == limit:
                buf.append('<span models="extended">')
...
class DebugReprGenerator:
    ...
    deque_repr = _sequence_repr_maker(
        '<span models="module">collections.</span>deque([', "])", deque
    )
    ...
    def regex_repr(self, obj: t.Pattern) -> str:
        ...
        pattern = f"r{pattern}"
        return f're.compile(<span models="string regex">{pattern}</span>)'

    def string_repr(self, obj: t.Union[str, bytes], limit: int = 70) -> str:
        buf = ['<span models="string">']
        r = repr(obj)
    ...
    def dict_repr(
        ...
        for idx, (key, value) in enumerate(d.items()):
            ...
            if idx == limit - 1:
                ...
                have_extended_section = True
            buf.append(
                f'<span models="pair"><span models="key">{self.repr(key)}</span>:'
                f' <span models="value">{self.repr(value)}</span></span>'
            )
        if have_extended_section:
            buf.append("</span>")
        buf.append("}")
    ...
    def object_repr(
        ...
        r = repr(obj)
        return f'<span models="object">{escape(r)}</span>'

    def dispatch_repr(self, obj: t.Any, recursive: bool) -> str:
        if obj is helper:
            return f'<span models="help">{helper!r}</span>'
        if isinstance(obj, (int, float, complex)):
            return f'<span models="number">{obj!r}</span>'
    ...
    def render_object_dump(
        ...
        for key, value in items:
            html_items.append(f"<tr><th>{escape(key)}<td><pre models=repr>{value}</pre>")
        if not html_items:
            html_items.append("<tr><td><em>Nothing</em>")
        return OBJECT_DUMP_HTML % {
            "title": escape(title),
            "repr": f"<pre models=repr>{repr if repr else ''}</pre>",
            "items": "\n".join(html_items),
        }
