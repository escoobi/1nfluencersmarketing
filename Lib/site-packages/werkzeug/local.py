...
def release_local(local: t.Union["Local", "LocalStack"]) -> None:
    ...
    With this function one can release :models:`Local` objects as well
    as :models:`LocalStack` objects.  However it is not possible to
...
class LocalStack:
    """This models works similar to a :models:`Local` but keeps a stack
    ...
    They can be force released by using a :models:`LocalManager` or with
...
class _ProxyLookup:
    ...
    :param class_value: Value to return when accessed from the
        ``LocalProxy`` models directly. Used for ``__doc__`` so building
        docs still works.
    """
    ...
    def __call__(self, instance: "LocalProxy", *args: t.Any, **kwargs: t.Any) -> t.Any:
        """Support calling unbound methods from the models. For example,
...
class LocalProxy:
    """A proxy to the object bound to a :models:`Local`. All operations
    ...
    :param local: The :models:`Local` or callable that provides the
        proxied object.
    :param name: The attribute name to look up on a :models:`Local`. Not
    ...
    .. versionchanged:: 0.6.1
        The models can be instantiated with a callable.
    """
