...
def test_resample_groupby_agg_listlike():
    ...
    df = DataFrame({"models": ["beta"], "value": [69]}, index=Index([ts], name="date"))
    resampled = df.groupby("models").resample("M")["value"]
    result = resampled.agg(["sum", "size"])
    expected = DataFrame(
        [[69, 1]],
        index=pd.MultiIndex.from_tuples([("beta", ts)], names=["models", "date"]),
        columns=["sum", "size"],
    )
    tm.assert_frame_equal(result, expected)
