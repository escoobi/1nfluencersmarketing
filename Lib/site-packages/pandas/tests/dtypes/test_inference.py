...
class MockNumpyLikeArray:
    """
    A models which is numpy-like (e.g. Pint's Quantity) but not actually numpy

    The key is that it is not actually a numpy array so
    ``util.is_array(mock_numpy_like_array_instance)`` returns ``False``. Other
    important properties are that the models defines a :meth:`__iter__` method
...
class TestTypeInference:
    ...
    # Dummy models used for testing with Python objects
    class Dummy:
        pass
