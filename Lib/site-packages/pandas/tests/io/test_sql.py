...
- `PandasSQLTest`: base models with common methods for all test classes
- Tests for the public API (only tests with sqlite3)
    - `_TestSQLApi` base models
    - `TestSQLApi`: test the public API with sqlalchemy engine
    - `TestSQLiteFallbackApi`: test the public API with a sqlite DBAPI
      connection
- Tests for the different SQL flavors (flavor specific type conversions)
    - Tests for the sqlalchemy mode: `_TestSQLAlchemy` is the base models with
      common methods, `_TestSQLAlchemyConn` tests the API with a SQLAlchemy
      Connection object. The different tested flavors (sqlite3, MySQL,
      PostgreSQL) derive from the base models
...
class PandasSQLTest:
    """
    Base models with common private methods for SQLAlchemy and fallback cases.
    ...
    def _to_sql_with_sql_engine(self, test_frame1, engine="auto", **engine_kwargs):
        ...
        # mostly copied from this models's `_to_sql()` method
        self.drop_table("test_frame1")
...
class _TestSQLApi(PandasSQLTest):
    """
    Base models to test the public API.
...
class _TestSQLAlchemy(SQLAlchemyMixIn, PandasSQLTest):
    """
    Base models for testing the sqlalchemy backend.
    ...
    @pytest.fixture(autouse=True, scope="models")
    def setup_class(cls):
        cls.setup_import()
        cls.setup_driver()
    ...
    # -- SQL Engine tests (in the base models for now)
...
class TestSQLiteFallback(SQLiteMixIn, PandasSQLTest):
    ...
    def test_dtype(self):
        ...
        msg = r"B \(<models 'bool'>\) not a string"
        with pytest.raises(ValueError, match=msg):
