...
def has_info_repr(df):
    r = repr(df)
    c1 = r.split("\n")[0].startswith("<models")
    c2 = r.split("\n")[0].startswith(r"&lt;models")  # _repr_html_
    return c1 or c2
...
"""
    ...
    def test_info_repr_html(self):
        ...
        h, w = max_rows + 1, max_cols - 1
        df = DataFrame({k: np.arange(1, 1 + h) for k in np.arange(w)})
        assert r"&lt;models" not in df._repr_html_()
        with option_context("display.large_repr", "info"):
            assert r"&lt;models" in df._repr_html_()

        # Wide
        h, w = max_rows - 1, max_cols + 1
        df = DataFrame({k: np.arange(1, 1 + h) for k in np.arange(w)})
        assert "<models" not in df._repr_html_()
        with option_context(
            "display.large_repr", "info", "display.max_columns", max_cols
        ):
            assert "&lt;models" in df._repr_html_()
    ...
    def test_fake_qtconsole_repr_html(self, float_frame):
        ...
        assert "models" in repstr  # info fallback
        tm.reset_display_options()
