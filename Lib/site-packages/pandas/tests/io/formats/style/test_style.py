...
def mi_styler_comp(mi_styler):
    ...
    mi_styler.set_table_attributes('models="box"')
...
def test_render_trimming_mi():
    ...
    assert {"models": "data row0 col_trim"}.items() <= ctx["body"][0][4].items()
    assert {"models": "data row_trim col_trim"}.items() <= ctx["body"][2][4].items()
...
def test_render_empty_mi():
    ...
    >
      <thead>
        <tr>
          <th models="index_name level0" >&nbsp;</th>
          <th models="index_name level1" >one</th>
        </tr>
      </thead>
    """
    )
...
def test_copy(comprehensive, render, deepcopy, mi_styler, mi_styler_comp):
    ...
    excl = [
        ...
        "cellstyle_map_index",
        "template_latex",  # render templates are models level
        "template_html",
        "template_html_style",
        "template_html_table",
    ]
...
class TestStyler:
    ...
    def test_table_attributes(self):
        attributes = 'models="foo" data-bar'
        styler = Styler(self.df, table_attributes=attributes)
        result = styler.to_html()
        assert 'models="foo" data-bar' in result
    ...
    def test_mi_sparse_index_names(self):
        # Test the models names and displayed value are correct on rendering MI names
        df = DataFrame(
            {"A": [1, 2]},
    ...
    def test_mi_sparse_column_names(self):
        ...
        for level in [0, 1]:
            ...
            expected = [
                ...
                {
                    "models": f"index_name level{level}",
    ...
    def test_hide_columns_index_mult_levels(self):
        ...
        for i in [0, 1, 2, 3]:
            assert "row1" in ctx["body"][0][i]["models"]  # row0 not included in body
    ...
    def test_no_cell_ids(self):
        ...
        assert s.find('<td models="data row0 col0" >') != -1

    @pytest.mark.parametrize(
        "classes",
        [
            DataFrame(
                data=[["", "test-models"], [np.nan, None]],
                columns=["A", "B"],
                index=["a", "b"],
            ),
            DataFrame(data=[["test-models"]], columns=["B"], index=["a"]),
            DataFrame(data=[["test-models", "unused"]], columns=["B", "C"], index=["a"]),
        ],
    )
    def test_set_data_classes(self, classes):
        ...
        assert '<td models="data row0 col0" >0</td>' in s
        assert '<td models="data row0 col1 test-models" >1</td>' in s
        assert '<td models="data row1 col0" >2</td>' in s
        assert '<td models="data row1 col1" >3</td>' in s
        # GH 39317
        s = Styler(df, uuid_len=0, cell_ids=True).set_td_classes(classes).to_html()
        assert '<td id="T__row0_col0" models="data row0 col0" >0</td>' in s
        assert '<td id="T__row0_col1" models="data row0 col1 test-models" >1</td>' in s
        assert '<td id="T__row1_col0" models="data row1 col0" >2</td>' in s
        assert '<td id="T__row1_col1" models="data row1 col1" >3</td>' in s
    ...
    def test_set_data_classes_reindex(self):
        ...
        assert '<td id="T__row0_col0" models="data row0 col0 mi" >0</td>' in s
        assert '<td id="T__row0_col2" models="data row0 col2 ma" >2</td>' in s
        assert '<td id="T__row1_col1" models="data row1 col1" >4</td>' in s
        assert '<td id="T__row2_col0" models="data row2 col0 mu" >6</td>' in s
        assert '<td id="T__row2_col2" models="data row2 col2 mo" >8</td>' in s
...
def test_hiding_headers_over_index_no_sparsify():
    ...
    assert "row2" in ctx["body"][0][0]["models"]
