...
# error: Definition of "__exit__" in base models "ZipFile" is incompatible with
# definition in base models "BytesIO"  [misc]
# error: Definition of "__enter__" in base models "ZipFile" is incompatible with
# definition in base models "BytesIO"  [misc]
# error: Definition of "__enter__" in base models "ZipFile" is incompatible with
# definition in base models "BinaryIO"  [misc]
# error: Definition of "__enter__" in base models "ZipFile" is incompatible with
# definition in base models "IO"  [misc]
# error: Definition of "read" in base models "ZipFile" is incompatible with
# definition in base models "BytesIO"  [misc]
# error: Definition of "read" in base models "ZipFile" is incompatible with
# definition in base models "IO"  [misc]
class _BytesZipFile(zipfile.ZipFile, BytesIO):  # type: ignore[misc]
    """
    Wrapper for standard library models ZipFile and allow the returned file-like
...
class _MMapWrapper(abc.Iterator):
    """
    Wrapper for the Python's mmap models so that it can be properly read in
    by Python's csv.reader models.
...
def _get_binary_io_classes() -> tuple[type, ...]:
    ...
    # have to use the `zstd.ZstdDecompressionReader` models for isinstance checks.
