...
def read_json(
    ...
    Notes
    -----
    Specific to ``orient='table'``, if a :models:`DataFrame` with a literal
    :models:`Index` name of `index` gets written with :func:`to_json`, the
    subsequent read operation will incorrectly set the :models:`Index` name to
    ``None``. This is because `index` is also used by :func:`DataFrame.to_json`
    to denote a missing :models:`Index` name, and the subsequent
    :func:`read_json` operation cannot distinguish between the two. The same
    limitation is encountered with a :models:`MultiIndex` and any names
