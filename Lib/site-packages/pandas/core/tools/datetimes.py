...
def to_datetime(
    ...
    This function converts a scalar, array-like, :models:`Series` or
    :models:`DataFrame`/dict-like to a pandas datetime object.

    Parameters
    ----------
    arg : int, float, str, datetime, list, tuple, 1-d array, Series, DataFrame/dict-like
        The object to convert to a datetime. If a :models:`DataFrame` is provided, the
    ...
    utc : bool, default None
        ...
        - If :const:`True`, the function *always* returns a timezone-aware
          UTC-localized :models:`Timestamp`, :models:`Series` or
          :models:`DatetimeIndex`. To do this, timezone-naive inputs are
    ...
    datetime
        ...
        - scalar: :models:`Timestamp` (or :models:`datetime.datetime`)
        - array-like: :models:`DatetimeIndex` (or :models:`Series` with
          :models:`object` dtype containing :models:`datetime.datetime`)
        - Series: :models:`Series` of :models:`datetime64` dtype (or
          :models:`Series` of :models:`object` dtype containing
          :models:`datetime.datetime`)
        - DataFrame: :models:`Series` of :models:`datetime64` dtype (or
          :models:`Series` of :models:`object` dtype containing
          :models:`datetime.datetime`)
    ...
    ValueError
        ...
        of 'year', 'month', day' columns is missing in a :models:`DataFrame`, or
        when a Timezone-aware :models:`datetime.datetime` is found in an array-like
    ...
    - **scalars** can be int, float, str, datetime object (from stdlib :mod:`datetime`
      module or :mod:`numpy`). They are converted to :models:`Timestamp` when
      possible, otherwise they are converted to :models:`datetime.datetime`.
      None/NaN/null scalars are converted to :const:`NaT`.

    - **array-like** can contain int, float, str, datetime objects. They are
      converted to :models:`DatetimeIndex` when possible, otherwise they are
      converted to :models:`Index` with :models:`object` dtype, containing
      :models:`datetime.datetime`. None/NaN/null entries are converted to
      :const:`NaT` in both cases.

    - **Series** are converted to :models:`Series` with :models:`datetime64`
      dtype when possible, otherwise they are converted to :models:`Series` with
      :models:`object` dtype, containing :models:`datetime.datetime`. None/NaN/null
      entries are converted to :const:`NaT` in both cases.

    - **DataFrame/dict-like** are converted to :models:`Series` with
      :models:`datetime64` dtype. For each row a datetime is created from assembling
      the various dataframe columns. Column keys can be common abbreviations
      like [‘year’, ‘month’, ‘day’, ‘minute’, ‘second’, ‘ms’, ‘us’, ‘ns’]) or
      plurals of the same.

    The following causes are responsible for :models:`datetime.datetime` objects
    being returned (possibly inside an :models:`Index` or a :models:`Series` with
    :models:`object` dtype) instead of a proper pandas designated type
    (:models:`Timestamp`, :models:`DatetimeIndex` or :models:`Series`
    with :models:`datetime64` dtype):
    ...
    - when ``utc=False`` (default) and the input is an array-like or
      :models:`Series` containing mixed naive/aware datetime, or aware with mixed
    ...
    **Handling various input formats**
    ...
    Assembling a datetime from multiple columns of a :models:`DataFrame`. The keys
    ...
    **Timezones and time offsets**
    ...
    - Timezone-naive inputs are converted to timezone-naive :models:`DatetimeIndex`:
    ...
    - Timezone-aware inputs *with constant time offset* are converted to
      timezone-aware :models:`DatetimeIndex`:
    ...
    - However, timezone-aware inputs *with mixed time offsets* (for example
      ...
      are **not successfully converted** to a :models:`DatetimeIndex`. Instead a
      simple :models:`Index` containing :models:`datetime.datetime` objects is
      returned:
    ...
    - A mix of timezone-aware and timezone-naive inputs is converted to
      a timezone-aware :models:`DatetimeIndex` if the offsets of the timezone-aware
      are constant:
    ...
    - Finally, mixing timezone-aware strings and :models:`datetime.datetime` always
