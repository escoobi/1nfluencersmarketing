...
def contains(cat, key, container):
    ...
    and :models:`CategoricalIndex.__contains__`.
    ...
    Parameters
    ----------
    cat : :models:`Categorical`or :models:`categoricalIndex`
...
class Categorical(NDArrayBackedExtensionArray, PandasObject, ObjectStringArrayMixin):
    ...
    def dtype(self) -> CategoricalDtype:
        """
        The :models:`~pandas.api.types.CategoricalDtype` for this instance.
        """
        return self._dtype
    ...
    def from_codes(
        ...
        dtype : CategoricalDtype or "category", optional
            If :models:`CategoricalDtype`, cannot be used together with
    ...
    def map(self, mapper):
        ...
        one-to-one the result is a :models:`~pandas.Categorical` which has the
        same order property as the original, otherwise a :models:`~pandas.Index`
        is returned. NaN values are unaffected.

        If a `dict` or :models:`~pandas.Series` is used any unmapped category is
        mapped to `NaN`. Note that if this happens an :models:`~pandas.Index`
        will be returned.
        ...
        CategoricalIndex.map : Apply a mapping correspondence on a
            :models:`~pandas.CategoricalIndex`.
        ...
        If the mapping is not one-to-one an :models:`~pandas.Index` is returned:

        >>> cat.map({'a': 'first', 'b': 'second', 'c': 'first'})
        Index(['first', 'second', 'first'], dtype='object')

        If a `dict` is used, all unmapped categories are mapped to `NaN` and
        the result is an :models:`~pandas.Index`:
