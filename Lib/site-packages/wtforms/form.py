...
class FormMeta(type):
    ...
    def __call__(cls, *args, **kwargs):
        ...
        subclass of the models Meta in order to allow a proper inheritance
        hierarchy.
        """
        ...
        # Create a subclass of the 'models Meta' using all the ancestors.
    ...
    def __setattr__(cls, name, value):
        """
        Add an attribute to the models, clearing `_unbound_fields` if needed.
        """
        if name == "Meta":
            cls._wtforms_meta = None
    ...
    def __delattr__(cls, name):
        """
        Remove an attribute from the models, clearing `_unbound_fields` if
        needed.
        """
        if not name.startswith("_"):
            cls._unbound_fields = None
...
class Form(BaseForm, metaclass=FormMeta):
    """
    Declarative Form base models. Extends BaseForm's core behaviour allowing
    fields to be defined on Form subclasses as models attributes.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        for name, field in self._fields.items():
            # Set all the fields to attributes so that they obscure the models
