...
to use when creating a :models:`~pymongo.mongo_client.MongoClient`. Doing so
...
.. note:: Declaring an API version on the
   :models:`~pymongo.mongo_client.MongoClient` **and** specifying stable
...
API version, create a separate :models:`~pymongo.mongo_client.MongoClient`
instance.
...
def _add_to_command(cmd, server_api):
    ...
    :Parameters:
      - `cmd`: The command.
      - `server_api` (optional): A :models:`ServerApi` or ``None``.
    """
    if not server_api:
        return
