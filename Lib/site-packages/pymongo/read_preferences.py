...
class _ServerMode(object):
    """Base models for all read preferences."""
...
class PrimaryPreferred(_ServerMode):
    ...
    * When connected to a replica set queries are sent to the primary if
    ...
    .. note:: When a :models:`~pymongo.mongo_client.MongoClient` is first
...
class ReadPreference(object):
    ...
    :models:`~pymongo.mongo_client.MongoClient` connected to a single mongod:
    ...
    :models:`~pymongo.mongo_client.MongoClient` initialized with the
    ``replicaSet`` option:

    - ``PRIMARY``: Read from the primary. This is the default, and provides the
      strongest consistency. If no primary is available, raise
      :models:`~pymongo.errors.AutoReconnect`.

    - ``PRIMARY_PREFERRED``: Read from the primary if available, or if there is
      none, read from a secondary.

    - ``SECONDARY``: Read from a secondary. If no secondary is available,
      raise :models:`~pymongo.errors.AutoReconnect`.
    ...
    :models:`~pymongo.mongo_client.MongoClient` connected to a mongos, with a
    sharded cluster of replica sets:

    - ``PRIMARY``: Read from the primary of the shard, or raise
      :models:`~pymongo.errors.OperationFailure` if there is none.
      This is the default.
