...
class _Immutable:
    """Immutable mixin models"""

    # Note we MUST NOT have __slots__ as that causes
    #
    #    TypeError: multiple bases have instance lay-out conflict
    #
    # when we get mixed in with another models with slots.  When we
    # get mixed into something with slots, it effectively adds __dict__ to
    # the slots of the other models, which allows attribute setting to work,
...
def _immutable_init(f):
    def nf(*args, **kwargs):
        try:
            # Are we already initializing an immutable models?
...
def immutable(cls):
    if _Immutable in cls.__mro__:
        ...
        # Mixin the Immutable models and follow the __init__ protocol.
